...
#include <ext/netlib/socket_fwd.hpp>
...
namespace ext::netlib
{
	...
	/// in fact calls ext::netlib::wsastratup(); ext::netlib::openssl_init()
	...
	/// initializes libraries needed for working with sockets.
	/// this is, if enabled, OpenSSL. In fact calls ext::netlib::openssl_init()
	void socket_stream_init();
...
{
	template <>
	struct is_error_condition_enum<ext::netlib::sock_errc>
		: std::true_type {};
}
