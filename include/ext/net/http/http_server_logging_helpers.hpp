...
#define SOCK_LOG_FATAL(f, ...) EXTLL_FATAL_STR(m_logger, fmt::format("sock={}, " f, context->sock.handle(), ##__VA_ARGS__))
#define SOCK_LOG_ERROR(f, ...) EXTLL_ERROR_STR(m_logger, fmt::format("sock={}, " f, context->sock.handle(), ##__VA_ARGS__))
#define SOCK_LOG_WARN(f, ...) EXTLL_WARN_STR(m_logger, fmt::format("sock={}, " f, context->sock.handle(), ##__VA_ARGS__))
#define SOCK_LOG_INFO(f, ...) EXTLL_INFO_STR(m_logger, fmt::format("sock={}, " f, context->sock.handle(), ##__VA_ARGS__))
#define SOCK_LOG_DEBUG(f, ...) EXTLL_DEBUG_STR(m_logger, fmt::format("sock={}, " f, context->sock.handle(), ##__VA_ARGS__))
#define SOCK_LOG_TRACE(f, ...) EXTLL_TRACE_STR(m_logger, fmt::format("sock={}, " f, context->sock.handle(), ##__VA_ARGS__))
